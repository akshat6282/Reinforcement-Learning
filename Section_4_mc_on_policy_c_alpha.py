...
def on_policy_mc_control_c_alpha(episodes=20, gamma=0.9, alpha=0.1, wantToView=False):
    ...
    for i in range(1, episodes + 1):
        ...
        for step in range(len(episode) - 1, -1, -1): 
            ...
            G = reward + gamma * G
            qsa = q[(state, action)]
            q[(state, action)] += alpha*(G - qsa)

    print("Training complete")

on_policy_mc_control_c_alpha(wantToView=True)